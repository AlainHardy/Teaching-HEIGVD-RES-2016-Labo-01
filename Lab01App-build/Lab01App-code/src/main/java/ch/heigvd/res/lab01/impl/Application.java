...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    // If quotes has been added, then remove the last /, in order to have a correct path
    if(quote.getTags() != null) {
      path = path.substring(0, path.length() - 1);
    }

    File directory = new File(WORKSPACE_DIRECTORY, path);

    // Check if the directory is really a directory and if it already exists
    if (!directory.isDirectory() && !directory.mkdirs()) {
      throw new IOException("Unable to create directory!");
    }


    // Creation of File instance and FileWriter to write the whole quote in the file
